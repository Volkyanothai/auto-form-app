...
.stApp{
  background:
    ...
    linear-gradient(170deg,#050a18 0%,#0a1228 48%,#060c1c 100%);
...
.hero{
  ...
  border:1px solid rgba(140,175,255,.16);border-radius:28px;
...
.hero h1{
  font-size:2.7rem;font-weight:800;letter-spacing:4px;margin:0 0 .45rem;
  background:linear-gradient(100deg,#fff 10%,#7fb3ff 45%,#e8c98a 92%);
...
div[data-testid="stVerticalBlockBorderWrapper"]:hover{
  ...
  border-color:rgba(127,179,255,.38);
  box-shadow:0 22px 60px rgba(0,0,0,.55),0 0 26px rgba(127,179,255,.14);
}
@keyframes rise{from{opacity:0;transform:translateY(22px)}to{opacity:1;transform:none}}

.glass-header{
  font-size:.78rem;font-weight:700;letter-spacing:1.4px;color:#7fb3ff;
...
div.stButton>button{
  ...
  background:linear-gradient(135deg,#7fb3ff,#a9cdff 45%,#e8c98a);
...
div.stButton>button:hover{
  ...
  box-shadow:0 16px 40px rgba(127,179,255,.45);
}
...
.stTextInput input:focus,.stTextArea textarea:focus{
  border-color:#7fb3ff;
  box-shadow:0 0 0 3px rgba(127,179,255,.18);
}
.stTextInput label p,.stSelectbox label p,.stTextArea label p{
  color:#93a7cc;font-size:.72rem;font-weight:700;
  letter-spacing:1px;text-transform:uppercase;
}
.stTextInput input:disabled{
  color:#eaf0ff;-webkit-text-fill-color:#eaf0ff;opacity:1;
}
...
.reasoning-text{
  ...
  border-left:3px solid #7fb3ff;margin-bottom:12px;
}
...
.score-val{
  ...
  background:linear-gradient(120deg,#e8c98a,#fff,#7fb3ff);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;
}
...
.score-link{
  ...
  color:#7fb3ff;text-decoration:none;font-weight:700;letter-spacing:.8px;
  margin-top:10px;transition:all .3s ease;
}
.score-link:hover{background:rgba(127,179,255,.20);transform:translateY(-2px);}

.section-title{
  font-size:1.35rem;font-weight:700;color:#eaf0ff;
  margin:2rem 0 1rem;letter-spacing:.5px;
}
[data-testid="stStatusWidget"]{
  background:rgba(14,26,54,.75);
  border:1px solid rgba(140,175,255,.16);
  border-radius:16px;
}
[data-testid="stExpander"]{
  background:rgba(14,26,54,.5);
  border:1px solid rgba(140,175,255,.16);
  border-radius:16px;
}
...
@media (max-width:640px){
  ...
  div[data-testid="stVerticalBlockBorderWrapper"]{padding:1.05rem;border-radius:18px;}
...
def render_header(title: str = "EZEXAM", subtitle: str = "AUTO FORM SYSTEM") -> None:
    ...
    st.markdown(
        '<div class="hero">' + mark + '<h1>' + title + '</h1><p>' + subtitle + '</p>'
